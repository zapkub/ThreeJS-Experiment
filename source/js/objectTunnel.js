...
app.controller('mainController', ['$scope', '$timeout', '$http', function($scope, $timeout, $http) {
  ...
  loader.crossOrigin = "http://www.gth.co.th";

  $http.get('http://www.gth.co.th/farewell/api/imageCurl.php?url=' + 'http://www.gth.co.th/farewell/api/media/682223042576027648.jpg').then(function(data) {
    console.log(data);
  });
  ...
  var distanceOfPostZAxis = 200;

  if (isMobile()) {
    distanceOfPostZAxis = 500;

  }
  var posts = [];
  var rawPostData = [];
  $scope.timeSlider = 400;
  ...
  var init = function() {
    ...
    glCamera = new THREE.PerspectiveCamera(90, window.innerWidth / window.innerHeight, 1, viewDepth + 1000);
    ...
    for (var i = 0; i < numberOfPosts; i++) {
      ...
      var x = Math.random() * 500 - 250; //(200 + randomX) * factorX + ( -400 + randomX) * !factorX;
    ...
    var geo = new THREE.PlaneGeometry(600, 350);
    var mat = new THREE.MeshBasicMaterial({
      color: 'white',
      side: THREE.DoubleSide,
      transparent: true,
      map: loader.load('images/hud/ogimage.jpg')
    })

    var mesh = new THREE.Mesh(geo, mat);
    mesh.position.z = -distanceOfPostZAxis * i;
  ...
  $http.get('http://www.gth.co.th/farewell/api/FetchAllApi.php').then(
    function(res) {
      ...
      $scope.$watch('timeSlider', function(newVal, oldVal) {
        ...
        if (controls) {
          controls.target.set(0, 0, -$scope.timeSlider * distanceOfPostZAxis + 1000)
          glCamera.position.z = -$scope.timeSlider * distanceOfPostZAxis + 1500
        ...
        promise = $timeout(function() {
          ...
          _.each(posts, function(object) {
            ...
            if (frustum.containsPoint(object.mesh.position) && !object.isLoaded) {
              loader.load('http://www.gth.co.th/farewell/api/media/' + object.data.id + '.jpg?rand=' + Math.random(), function(texture) {
